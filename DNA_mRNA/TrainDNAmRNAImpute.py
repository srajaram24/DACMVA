...
def get_lamb(currEpoch,maxEpochs):
    ...
    lamb=0.01*min(1.0,schedulePt/(cycleLen/2))
...
class TrainDNAmRNAImputeClass:
    def __init__(self,config,save_dir,max_epochs,missingPercent,dataloaderDir,gpu,dataLocationTest,modelLocation=None,modelLocEpoch=-1,fixedConfig=False):
        ...
        folderName="lrAE_{}_lrD_{}_bs_{}_beta_{}_alpha_{}_gam_{}_lrMS_{}_t_{}".format(config["lrAE"],config["lrD"],config["batch_size"],config["beta"],config["alpha"],config["gamma"],config["lrMS"],config["t"])
        ...
        self.criterion_gan = nn.MSELoss() #nn.CrossEntropyLoss()
