...
from DNA_mRNA.TrainDNAmRNAImputeLateMS import TrainDNAmRNAImputeLateMSClass
...
def trial_str_creator(trial):
    config=trial.config
    return "lrAE_{}_lrD_{}_bs_{}_beta_{}_alpha_{}_gam_{}_lrMS_{}_t_{}".format(config["lrAE"],config["lrD"],config["batch_size"],config["beta"],config["alpha"],config["gamma"],config["lrMS"],config["t"])
...
def runTrainMultiSurv(config,save_dir,max_epochs,missingPercent,dataloaderDir,gpu,dataLocationTest,modelLocation,modelLocEpoch,fixedConfig=False):
    MultisurvTrainer=TrainDNAmRNAImputeLateMSClass(config,save_dir,max_epochs,missingPercent,dataloaderDir,gpu,dataLocationTest,modelLocation,modelLocEpoch,fixedConfig)    
