...
# def get_lamb(currEpoch,maxEpochs):
#     numCycles=4
#     cycleLen=int(maxEpochs/numCycles)
#     schedulePt=currEpoch%cycleLen
#     lamb=.01*min(1.0,schedulePt/(cycleLen/2))
...
def get_lamb(currEpoch,maxEpochs):
    halfCyc=maxEpochs//2
    lamb=0.01*min(1.0,currEpoch/halfCyc)
...
class TrainDNAmRNAImputeLateMSClass:
    def __init__(self,config,save_dir,max_epochs,missingPercent,dataloaderDir,gpu,dataLocationTest,modelLocation=None,modelLocEpoch=-1,fixedConfig=False):
        ...
        self.fixedConfig=fixedConfig #here
        folderName="lrAE_{}_lrD_{}_bs_{}_beta_{}_alpha_{}_gam_{}_lrMS_{}_t_{}_Cyclic01MSLate2Cyc_MSbs_{}".format(config["lrAE"],config["lrD"],config["batch_size"],config["beta"],config["alpha"],config["gamma"],config["lrMS"],config["t"],config["MSbs"])
        ...
        self.netEncA = FC(5000, config["latent_dims"], 5, scaling_factor=2,VAE=config["VAE"]) 
        self.netDecA = DNADecoder(inFeatures=512) 
        self.netEncB = MRNAEncoder(in_features=1000,latentdim=config["latent_dims"],simple=True,VAE=config["VAE"])#MRNAEncoder(in_features=1000,latentdim=config["latent_dims"])
    ...
    def createImputations(self,dataloaderDNATrain):
        ...
        numAttempts=0
        while len(imputedBatch)<self.config["MSbs"] and numAttempts<len(dnaTrainDataset):
            numAttempts+=1
    ...
    def trainCM(self):
        ...
        if self.dataloaderDir is not None:
            ...
            with open(os.path.join(self.dataloaderDir,"dataloaderAll"+"RNATrain"+str(self.config["MSbs"])+"mp"+str(int(self.missingPercent*100))), 'rb') as file:
                dataloaderRNATrain = pickle.load(file)
        else:
        ...
        for epoch in range(self.max_epochs):
            print(epoch)
            self.config["lamb"]=get_lamb(epoch,self.max_epochs)
            ...
            if epoch>((self.max_epochs/2)+5):
                for idx, batch in enumerate(dataloaderRNATrain):
            ...
            self.writer.add_scalar("klWeight", self.config["lamb"], epoch)
            ...
            if epoch==((self.max_epochs/4)+5): 
                weightDirFirstCyc=os.path.join(self.save_dir,"modelfilesAfterFirstCycle") # here
                bestConcordMS=concordValMS
                bestEpoch=epoch
                if os.path.exists(weightDirFirstCyc):
                    shutil.rmtree(weightDirFirstCyc)
                os.makedirs(weightDirFirstCyc, exist_ok=True)
                torch.save(self.netEncA.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netEncA.pth"))
                torch.save(self.netEncB.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netEncB.pth"))
                torch.save(self.netCondClf.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netCondClf.pth"))
                torch.save(self.netDecA.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netDecA.pth"))
                torch.save(self.netDecB.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netDecB.pth"))
                torch.save(self.netClf.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netClf.pth"))
                torch.save(self.netMSEnc.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netMSEnc.pth"))
                torch.save(self.netMSClf.cpu().state_dict(), os.path.join(weightDirFirstCyc,"netMSClf.pth"))
