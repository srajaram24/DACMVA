...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 11 21:37:05 2023
...
def runTrainMultiSurv(config,save_dir,max_epochs,missingPercent,dataloaderDir,gpu,dataLocationTest):
    MultisurvTrainer=TrainOversamplingClass(config,save_dir,max_epochs,missingPercent,dataloaderDir,gpu,dataLocationTest)    
...
def fitmRNA(save_dir,max_epochs,missingPercent,dataloaderDir=None,gpu=False,dataLocationTest=None):
    ...
    for bs in [4]:
        for g in [.1,0.2,.3,.4,.5,0.6,.7,.8,.9]:
            for bsOV in [4,8,16,24,32]:
                thisConfig["batch_size"]=bs
                thisConfig["gamma"]=g
                thisConfig["bs_Oversample"]=bsOV
    ...
    # tVals=[2e10]
    # gammaVals=[0.2,0.6,0.7,0.8,0.9,1.0]
    # for t in tVals:
