...
from DNA_mRNA.TrainDNAmRNAVAE import TrainDNAmRNAVAEClass
...
def trial_str_creator(trial):
    ...
    return "lrAE_{}_lrD_{}_bs_{}_beta_{}_alpha_{}_lamb_{}".format(thisConfig["lrAE"],thisConfig["lrD"],thisConfig["batch_size"],thisConfig["beta"],thisConfig["alpha"],thisConfig["lamb"])
...
def runTrainMultiSurv(config,save_dir,save_freq,max_epochs,missingPercent,dataloaderDir,use_gpu,modelLocation,modelLocEpoch,fixedConfig=False):
    MultisurvTrainer=TrainDNAmRNAVAEClass(config,save_dir,save_freq,max_epochs,missingPercent,dataloaderDir,use_gpu,modelLocation,modelLocEpoch,fixedConfig)    
    MultisurvTrainer.trainCM()

def main(save_dir,save_freq,max_epochs,fixedConfig=False,missingPercent=0.0,dataloaderDir=None,use_gpu=False,modelLocation=None,modelLocEpoch=-1):
...
if __name__ == '__main__':
    ...
    if not torch.cuda.is_available():
        args.use_gpu = False
    main(args.save_dir,args.save_freq,args.max_epochs,fixedConfig=args.fixedConfig,
